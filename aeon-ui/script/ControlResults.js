...
let ControlResults = {
  ...
  initTable: {
    ...
    _fillTable(perts) {
      ...
      for (const pert of perts) {
        ...
        this._appendCell(
          row,
          ControlResults._createColouredVars(pert.perturbation),
          this._perturbationToString(pert.perturbation),
          "70%",
        );
        this._appendCell(
          row,
          Object.keys(pert.perturbation).length,
          null,
          "6%",
        );
        ...
        this._appendCell(
          row,
          pert.robustness < 0.0001
            ? "<0.00%"
            : (pert.robustness * 100).toFixed(2),
          null,
          "5%",
        );
  ...
  sortTable: {
    ...
    _comparator(a, b, primaryIndex, secondaryIndex, orderLevelIndex) {
      ...
      const result =
        a.getElementsByTagName("td")[primaryIndex].textContent -
      ...
      const reverseOrder = this._orderArrows[
        orderLevelIndex
    ...
    _sortRows() {
      ...
      const sortedRows = Array.from(ControlResults._perturbTable.rows).sort(
        (a, b) => this._comparator(a, b, primaryIndex, secondaryIndex, 0),
      );
    ...
    switchSortMode(buttonID) {
      ...
      if (
        this._sortMode[0].value == "ID" ||
        this._sortMode[0].value == this._sortMode[1].value
      ) {
  ...
  filterTable: {
    ...
    _includesPerts(perts, include) {
      for (const variable in include) {
        if (
          include[variable] != "ignore" &&
          (perts[variable] == undefined ||
            (include[variable] != "present" &&
              perts[variable] != include[variable]))
        ) {
          return false;
        }
      }
    ...
    filterTable() {
      ...
      for (const row of ControlResults._perturbTable.rows) {
        ...
        if (
          !this._includesPerts(
            JSON.parse(row.getAttribute("perturb")),
            filterValues[0],
          ) ||
  ...
  init() {
    this._perturbTable = document
      .getElementById("control-results-table")
      .getElementsByTagName("tbody")[0];

    this.filterTable._filters = [
      document.getElementById("controlr-size-filter"),
      document.getElementById("controlr-NoI-filter"),
      document.getElementById("controlr-rob-filter"),
    ];

    this.sortTable._sortMode = [
      document.getElementById("primary-sort-switch"),
      document.getElementById("secondary-sort-switch"),
    ];
    this.sortTable._orderArrows = [
      document.getElementById("control-res-primary-arrow"),
      document.getElementById("control-res-secondary-arrow"),
    ];
    this._filterMenu = document.getElementById("control-res-filters");
    this.filterTable._pertFilterTable = new TableWidget(
      document.getElementById("control-pert-filter-table"),
  ...
  insertData(controlData) {
    ...
    document.getElementById("InterNumberStat").textContent =
      controlData.stats.allColorsCount;
    document.getElementById("PertNumberStat").textContent =
      controlData.stats.perturbationCount;
    document.getElementById("MinSizeStat").textContent =
      controlData.stats.minimalPerturbationSize;
    document.getElementById("MaxRobStat").textContent =
      controlData.stats.maximalPerturbationRobustness < 0.0001
        ? "<0.00%"
        : `${(controlData.stats.maximalPerturbationRobustness * 100).toFixed(2)}%`;
    document.getElementById("OscillationStat").textContent =
      controlData.stats.oscillation;
    document.getElementById("PhenotypeStat").innerHTML =
      this._createColouredVars(controlData.stats.phenotype);
    document.getElementById("ControllableStat").textContent =
      controlData.stats.controllable;

    for (variable of controlData.stats.controllable) {
      this.filterTable._pertFilterValues[variable] = "ignore";
      this.filterTable._pertFilterTable.addVariable(
        variable,
        variable,
        "phenotype",
      );
  ...
  changeContentModeAllPerts(button) {
    ...
    button.setAttribute(
      "colorShown",
      !(button.getAttribute("colorShown") == "true"),
    );
  },

  /** Toggles visibility of the ControlResults._filterMenu. */
  toggleFilters() {
    this._filterMenu.style.display =
      this._filterMenu.style.display == "none" ? "" : "none";
