...
let CytoscapeTreeEditor = {
  ...
  getParentNode(targetId) {
    let parentEdge = CytoscapeTreeEditor._cytoscape.edges(
  ...
  getChildNode(sourceId, positive) {
    let childEdge = CytoscapeTreeEditor._cytoscape.edges(
  ...
  _showMixedPanel(data) {
    ...
    addButton.onclick = function () {
      CytoscapeTreeEditor._makeDecisionFunction(addButton, loading, data);
    };
  ...
  _computeMassOpacity(cardinality) {
    ...
    let percent = TreeExplorer.Math_dimPercent(
      cardinality,
      this._totalCardinality,
    );
    ...
    const options = settings.useTidytree
      ? {
          ...
          edgeComparator: (e1, e2) => {
            const order =
              (e1.data().positive === "true") - (e2.data().positive === "true");
