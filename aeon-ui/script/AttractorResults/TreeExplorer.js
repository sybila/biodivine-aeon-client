...
let TreeExplorer = {
  openNewTreeExplorer() {
    ...
    depth.oninput = function () {
      ...
      if (value == 1) {
        autoExpand.innerHTML =
          "Auto expand (1 level)  <img src='img/graph-24px.svg'>";
      } else {
        autoExpand.innerHTML =
          "Auto expand (" + value + " levels)  <img src='img/graph-24px.svg'>";
      }
    };

    autoExpand.onclick = function () {
      TreeExplorer._autoExpandBifurcationTree(
    ...
    document
      .getElementById("button-resetlayout")
  ...
  renderAttributeTable(id, attributes, totalCardinality) {
    ...
    for (attr of attributes) {
      ...
      nameText.onclick = new Function(
        "TreeExplorer._selectAttribute(" + id + ", " + attr.id + ")",
      );
      ...
      leftNode.getElementsByClassName("title")[0].innerHTML =
        ...
        attr.left.length +
        "|<small>" +
        this.Math_percent(leftTotal, totalCardinality) +
        "%</small>)";
      rightNode.getElementsByClassName("title")[0].innerHTML =
        "Positive (" +
        attr.right.length +
        "|<small>" +
        this.Math_percent(rightTotal, totalCardinality) +
        "%</small>)";
  ...
  openTreeWitness() {
    ...
    window.open(
      url +
        "?engine=" +
        encodeURI(ComputeEngine.Connection.getAddress()) +
        "&tree_witness=" +
        encodeURI(node),
    );
  },
          ...
          for (item of r) {
            ...
            if (item.data.length == 1) {
              content +=
                ...
                variableName +
                "</b>: always " +
                TreeExplorer._vector_to_string(item.data[0].vector) +
                "</div>";
            } else {
              ...
              for (data of item.data) {
                content +=
                  " - " +
                  TreeExplorer._vector_to_string(data.vector) +
                  ": " +
                  data.colors +
                  TreeExplorer._getWitnessPanelForVariable(
                    variableName,
                    behaviour,
                    data.vector,
                  ) +
  ...
  _getWitnessPanelForVariable(variable, behaviour, vector) {
    return (
      "<span class='witness-panel'><span class='inline-button' onclick='TreeExplorer.openStabilityWitness(\"" +
      variable +
      '","' +
      behaviour +
      '","' +
      vector +
      "\");'>Witness</span> | <span class='inline-button' onclick='TreeExplorer.openStabilityAttractor(\"" +
      variable +
      '","' +
      behaviour +
      '","' +
