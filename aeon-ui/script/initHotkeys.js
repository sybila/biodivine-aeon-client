...
let InitHotkeys = {
  ...
  initTreeHotkeys() {
    ...
    hotkeys("backspace", function (event, handler) {
      ...
      if (
        selected !== undefined &&
        CytoscapeTreeEditor.getNodeType(selected) == "decision"
      ) {
        event.preventDefault();
    ...
    hotkeys("h", { keyup: true }, function (event, handler) {
      if (event.type === "keydown") {
        document
          .getElementById("quick-help-tree-explorer")
    ...
    hotkeys("s", function (event, handler) {
      ...
      if (!panel.classList.contains("gone")) {
        InitHotkeys.fireEvent(
          document.getElementById("mixed-stability-analysis-button"),
          "click",
        );
      }
      ...
      if (!panel.classList.contains("gone")) {
        InitHotkeys.fireEvent(
          document.getElementById("decision-stability-analysis-button"),
          "click",
        );
      }
      ...
      if (!panel.classList.contains("gone")) {
        InitHotkeys.fireEvent(
          document.getElementById("leaf-stability-analysis-button"),
          "click",
        );
      }
    });
    ...
    hotkeys("d", function (event, handler) {
      ...
      if (!panel.classList.contains("gone")) {
        InitHotkeys.fireEvent(
          document.getElementById("make-decision-button"),
          "click",
        );
      }
    });
  ...
  initCanvasHotkeys() {
    hotkeys("e", function (event, handler) {
      if (UI.Visible.isNodeMenuVisible()) {
        event.preventDefault();
        InitHotkeys.fireEvent(
          document.getElementById("node-menu-edit-name"),
          "click",
        );
      }
    });
    ...
    hotkeys("f", function (event, handler) {
      if (UI.Visible.isNodeMenuVisible()) {
        event.preventDefault();
        InitHotkeys.fireEvent(
          document.getElementById("node-menu-edit-function"),
          "click",
        );
      }
    });
    ...
    hotkeys("c", function (event, handler) {
      const variable = LiveModel.Variables.variableFromId(
        CytoscapeEditor.getSelectedNodeId(),
      );

      if (variable != undefined && variable != null) {
        event.preventDefault();
        LiveModel.Control.changeControllableById(
          variable.id,
    ...
    hotkeys("o", function (event, handler) {
      if (UI.Visible.isEdgeMenuVisible()) {
        event.preventDefault();
        InitHotkeys.fireEvent(
          document.getElementById("edge-menu-observability"),
          "click",
        );
      }
    });
    ...
    hotkeys("m", function (event, handler) {
      if (UI.Visible.isEdgeMenuVisible()) {
        event.preventDefault();
        InitHotkeys.fireEvent(
          document.getElementById("edge-menu-monotonicity"),
          "click",
        );
      }
    });
