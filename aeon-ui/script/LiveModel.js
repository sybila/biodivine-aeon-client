...
let LiveModel = {
  ...
  Variables: {
      ...
      if (force || confirm(Strings.removeNodeCheck(variable["name"]))) {
        ...
        for (let id of update_regulations_after_delete) {
          ...
          if (LiveModel.UpdateFunctions._updateFunctions[id] !== undefined) {
            ...
            LiveModel.UpdateFunctions.setUpdateFunction(
              id,
              LiveModel.UpdateFunctions._updateFunctions[id].functionString,
            );
          }
          // And validate again.
          LiveModel.UpdateFunctions._validateUpdateFunction(id);
    ...
    renameVariable(id, newName) {
      ...
      if (error !== undefined) {
        ...
        for (var i = 0; i < LiveModel.Regulations._regulations.length; i++) {
          let reg = LiveModel.Regulations._regulations[i];
          if (reg.regulator == id || reg.target == id)
            LiveModel.Regulations._regulationChanged(reg);
        }
        LiveModel.Export.saveModel();
        return undefined;
      }
    ...
    pruneConstants(force = false) {
      ...
      for (const [id, variable] of Object.entries(this._variables)) {
        ...
        is_constant =
          is_constant & (LiveModel.Regulations.regulationsOf(id).length == 0);
        if (!force) {
          is_constant =
            is_constant &
            (LiveModel.UpdateFunctions._updateFunctions[id] === undefined);
        }
        if (is_constant) {
          to_remove.push(id);
        }
  ...
  UpdateFunctions: {
    ...
    _updateFunctionModelFragment(id) {
      ...
      for (var i = 0; i < regulations.length; i++) {
        if (regulations[i].regulator != id) {
          // if not self-loop, save
          varNames.add(
            LiveModel.Variables.getVariableName(regulations[i].regulator),
          );
        }
        fragment +=
          LiveModel.Regulations._regulationToString(regulations[i]) + "\n";
      }
    ...
    _checkUpdateFunction(id, functionString) {
      ...
      for (let item of names) {
        ...
        if (variable !== undefined) {
          let regulation = LiveModel.Regulations.findRegulation(
            variable.id,
            id,
          );
          if (regulation === undefined) {
            ...
            let message =
              ...
              "'.";
            if (
              confirm(message + " Do you want to create the regulation now?")
            ) {
      ...
      for (var i = 0; i < function_keys.length; i++) {
        ...
        for (let parameter of function_data.metadata.parameters) {
          for (let my_parameter of parameters) {
            if (
              parameter.name == my_parameter.name &&
              parameter.cardinality != my_parameter.cardinality
            ) {
              let message =
                "Parameter '" +
                my_parameter.name +
              ...
              message +=
                " Variable '" +
                LiveModel.Variables._variables[key].name +
  ...
  Regulations: {
    ...
    addRegulation(
      ...
      if (this.findRegulation(regulatorId, targetId) !== undefined)
        return false;
      let regulation = {
    ...
    _removeRegulation(regulation) {
      ...
      if (index > -1) {
        ...
        CytoscapeEditor.removeRegulation(
    ...
    _regulationToString(regulation) {
      let regulatorName = LiveModel.Variables.getVariableName(
        regulation.regulator,
      );
  ...
  Import: {
    ...
    _addVariableImport(variable, name, position, control) {
      ...
      return LiveModel.Variables.addVariable(
        true,
        position,
        name,
        control[0],
    ...
    _insertNotConnected(positions, control) {
      ...
      for (let variable of vars) {
        this._addVariableImport(
          LiveModel.Variables.variableFromName(variable),
          variable,
          positions[variable],
          control[variable],
        );
    ...
    _setRegulations(regulations, positions, control) {
      for (let template of regulations) {
        let regulator = this._addVariableImport(
          LiveModel.Variables.variableFromName(template.regulatorName),
          template.regulatorName,
          positions[template.regulatorName],
          control[template.regulatorName],
        );
        let target = this._addVariableImport(
          LiveModel.Variables.variableFromName(template.targetName),
          template.targetName,
    ...
    _setUpdateFunctions(updateFunctions, positions, control) {
      ...
      for (let key of keys) {
        let variable = this._addVariableImport(
          LiveModel.Variables.variableFromName(key),
          key,
          positions[key],
          control[key],
        );
    ...
    importAeon(modelString, erasePossible = false) {
      if (
        (!erasePossible && !LiveModel._modelModified()) ||
        (!LiveModel.isEmpty() &&
          !erasePossible &&
    ...
    stats() {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        if (LiveModel.UpdateFunctions._updateFunctions[key] === undefined) {
          ...
          let metadata =
            LiveModel.UpdateFunctions._updateFunctions[key].metadata;
    ...
    exportAeon(emptyPossible = false, withResults = true) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        for (var j = 0; j < regulations.length; j++) {
          result +=
            LiveModel.Regulations._regulationToString(regulations[j]) + "\n";
        }
      }
  ...
  _modelModified() {
    ...
    if (window.modelCalc[window.modelId] > 1) {
      Warning.displayWarning(
        "Model was modified: Id of the model will be changed from " +
          window.modelId +
          " to " +
          window.nextModelId.value,
      );
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (
        ...
        if (arg_tokens.length == 0) {
          // nullary function call - do nothing
          ...
          do {
            ...
            if (variable === undefined) {
              return (
                "Unknown argument '" +
                arg.data +
                "'. Only variables allowed as arguments."
              );
            }
            args.push(arg.data);
            j += 1;
