...
let ModelEditor = {
  ...
  Filter: {
      ...
      for (varName of targetVariables) {
        ...
        if (variable != undefined) {
          ...
          regulations.forEach((regulation) => {
            regulatorIDs.add(regulation.regulator);
          });
        }
      }
    ...
    filterVariables() {
      ...
      const variableNames = this._processFilterInput(
        findRegulators == true ? 9 : 0,
        filterValue,
      );
      const showAll =
      ...
      const regulatorIds =
        showAll == false && findRegulators == true
          ? this._getRegulators(variableNames)
          : undefined;
      ...
      LiveModel.Variables.getAllVariables().forEach((variable) => {
        ...
        if (showAll == true) {
          ...
          variableBox.style.display = regulatorIds.has(variable.id)
            ? ""
            : "none";
        } else {
          variableBox.style.display = this._includesToKeep(
            variableNames,
            variable.name,
          )
            ? ""
  ...
  init() {
    ...
    this._regulationTemplate = document.getElementById(
      "model-regulation-template",
    );
  ...
  addVariable(id, name) {
    ...
    let showVariableInfo =
      variableBox.getElementsByClassName("centered-button")[0];
    let variableControllability =
      variableBox.getElementsByClassName("centered-button")[1];
    let variablePhenotype =
      variableBox.getElementsByClassName("centered-button")[2];
    let variableInfo = variableBox.getElementsByClassName(
      "scrolling-container",
    )[0];
    let variableName = variableBox.getElementsByClassName("variable-name")[0];
    let updateFunction =
      variableBox.getElementsByClassName("variable-function")[0];
    ...
    variableControllability.style.backgroundColor = variable.controllable
      ? "#FFFF66"
      : "#B0BEC5";
    variablePhenotype.style.backgroundColor =
    ...
    showVariableInfo.addEventListener("click", (e) => {
      ...
      const variableBox = this._getVariableBox(
        e.target.getAttribute("variable-id"),
      );
      const variableInfo = variableBox.getElementsByClassName(
        "scrolling-container",
      )[0];
    ...
    variableControllability.addEventListener("click", (e) => {
      ...
      const variable = LiveModel.Variables.variableFromId(
        button.getAttribute("variable-id"),
      );
    ...
    updateFunction.addEventListener("focusout", (e) => {
      let error = LiveModel.UpdateFunctions.setUpdateFunction(
        id,
        updateFunction.textContent,
      );
      if (error !== undefined) {
        Warning.displayWarning(error);
    ...
    let variableShowButton = variableBox.getElementsByClassName(
      "model-variable-show",
    )[0];
    // Enable show button
    ...
    let removeVarButton = variableBox.getElementsByClassName(
  ...
  ensureRegulation(regulation) {
    ...
    if (variableBox !== undefined) {
      ...
      if (row === undefined) {
        ...
        variableBox
          .getElementsByClassName("model-variable-regulators")[0]
          .appendChild(row);
        let observable = row.getElementsByClassName(
          "model-regulation-observable",
        )[0];
        let monotonicity = row.getElementsByClassName(
      ...
      // Update row info...
      let regulatorName = row.getElementsByClassName(
        "model-regulation-regulator",
      )[0];
      let regulationShort = row.getElementsByClassName(
        "model-regulation-short",
      )[0];
      let observable = row.getElementsByClassName(
        "model-regulation-observable",
      )[0];
      ...
      regulatorName.textContent = LiveModel.Variables.getVariableName(
        regulation.regulator,
      );
      let short = "-";
  ...
  _getRegulatorRow(variableBox, regulatorId) {
    let regulators = variableBox.getElementsByClassName(
      "model-variable-regulators",
    )[0].children;
  ...
  updatePhenotype(variable) {
    const button = this._getVariableBox(variable.id).getElementsByClassName(
      "centered-button",
    )[2];
