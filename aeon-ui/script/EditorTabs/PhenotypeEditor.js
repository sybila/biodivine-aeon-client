...
let PhenotypeEditor = {
  ...
  init() {
    this._table = new TableWidget(
      document
        .getElementById("phenotype-editor-table")
        .getElementsByTagName("tbody")[0],
      document.getElementById("phenotype-editor-filter"),
    );

    this._counters = {
      true: new CountWidget(document.getElementById("phenotype-true-count"), 0),
      false: new CountWidget(
        document.getElementById("phenotype-false-count"),
        0,
      ),
      null: new CountWidget(
        document.getElementById("phenotype-notin-count"),
        0,
      ),
    };
  ...
  setSelected(phenValue) {
    for (row of this._table.table.rows) {
      if (row.classList.contains("selected")) {
        LiveModel.Control.changePhenotypeById(
