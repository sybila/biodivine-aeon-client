...
let TabBar = {
  ...
  init() {
    ...
    if (window.initialTabInfo.type == "tree-explorer") {
      ...
      this.addTab(
        window.initialTabInfo.type,
        JSON.parse(window.initialTabInfo.data),
      );
    }
  },
  ...
  addTab(type, data) {
    ...
    if (
      this._tabs[this._tabIds[type]] == undefined ||
      this._tabs[this._tabIds[type]] == null
    ) {
  ...
  toggleActive(newActiveId, returnEqual) {
    ...
    if (
      this._tabIds[newTab.type] == undefined ||
      this._tabIds[newTab.type] == null
    ) {
