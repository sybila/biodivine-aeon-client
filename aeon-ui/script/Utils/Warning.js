  ...
  _getWarningContent(warningType) {
    switch (warningType) {
      case "resultsAvailable":
        return {
          text: "There are existing results in the Results module. To proceed, you must either delete the results or open a copy of this model without results in a new browser tab.",
      ...
      case "tooManyControlRes":
        return {
          text: "You calculated more than 1000 results. Opening results in one tab may cause performance issues.",
          buttons: `<button class='centered-button' onclick='Warning.closeWarning()' style='height: 25px;'>Cancel</button>
      ...
      default:
        return {
          text: warningType,
          buttons:
            "<button class='centered-button' onclick='Warning.closeWarning()' style='height: 25px; margin: auto;'>Close</button>",
        };
    }
  },
