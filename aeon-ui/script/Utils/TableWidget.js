class TableWidget {
  ...
  _addRowElements(row, name, type) {
    ...
    if (type != null) {
      let hintMessage =
        type == "controllable"
          ? "Variable Controllable Status"
          : "Variable Phenotype Status";
      indicator.addEventListener("mouseenter", (e) => {
        UI.HoverHint.displayHover(e.target, 35, 0, hintMessage);
      });
    ...
    filterAdd.addEventListener("mouseenter", (e) => {
      UI.HoverHint.displayHover(e.target, 35, 0, "Add Into Filter");
    });
    filterAdd.addEventListener("mouseleave", () => {
      UI.HoverHint.hideHover();
    });
  ...
  filterTable() {
    ...
    const showAll =
      filterValues.length < 1 || filterValues[filterValues.length - 1] == "";
  ...
  changeSelectedAll(deselect) {
    for (let row of this.table.rows) {
      ...
      if (
        (deselect == null && !row.classList.contains("selected")) ||
        (deselect != null && !deselect)
      ) {
