...
let UI = {
  ...
  DownloadFile: {
    ...
    downloadSBMLInstantiated() {
      ...
      UI.isLoading(true);
      ComputeEngine.Format.aeonToSbmlInstantiated(
        modelFile,
        (error, result) => {
          UI.isLoading(false);
          if (error !== undefined) {
  ...
  Open: {
    ...
    openBrowserTab(withResults = true) {
      ...
      newWindow.lastComputation =
        withResults == true
          ? ComputeEngine.Computation.getLastComputation()
          : undefined;
  ...
  Visible: {
    ...
    showHideManual(manualId, headlineId, headline, textId) {
      ...
      if (textId == "" || manualElement.style.display == "") {
        ...
        document.getElementById(headlineId).textContent =
          "Manual - " + headline;

        if (manualElement.openText != undefined) {
          document.getElementById(manualElement.openText).style.display =
            "none";
        }
    ...
    toggleNodeMenu: function (position, zoom = 1.0) {
      ...
      if (position === undefined) {
        ...
        menu.style.transform =
          "scale(" + zoom * 0.75 + ") translate(-50%, -50%)";
      }
    },
  ...
  UpdateStatus: {
    ...
    _toggleResultsDownload(status, data) {
      ...
      if (
        data["timestamp"] !== null &&
        !(data.type == "control" && status != "done" && status != "cancelled")
      ) {
        cmpDownload.classList.remove("gone");
        if (status == "done") {
          cmpDownload.innerHTML =
            'Show result <img src="img/cloud_download-24px.svg">';
        } else {
          cmpDownload.innerHTML =
            'Show partial result <img src="img/cloud_download-24px.svg">';
        }
      } else {
    ...
    updateComputeEngineStatus(status, data) {
      ...
      if (status == "connected") {
        ...
        connectButton.innerHTML =
          "Disconnect <img src='img/cloud_off-24px.svg'>";
        if (data !== undefined) {
          ...
          if (data["is_running"] && data.progress != undefined) {
            ...
            if (status != "(none)") {
              const timeStatus =
                status +
                " " +
                this._getTime(data.timestamp, data["is_running"] == true);
          ...
          if (data["timestamp"] !== undefined && Results.hasResults()) {
            ...
            if (ComputeEngine.Computation.hasActiveComputation()) {
              ...
              document
                .getElementById("results-expired")
  ...
  init: function () {
    ...
    this._pageDivs = [
      document.getElementById("model-div"),
  ...
  _initEdgeMenu(menu) {
    ...
    monotonicity.updateState = function (data) {
      if (monotonicity.getAttribute("state") != data.monotonicity) {
        monotonicity.alt = monotonicity.getAttribute(
          "alt-" + data.monotonicity,
        );
        monotonicity.src = monotonicity.getAttribute(
          "src-" + data.monotonicity,
        );
