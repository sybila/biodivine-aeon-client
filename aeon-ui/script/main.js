...
function init() {
  ...
  if (
    window.initialTabInfo == undefined ||
  ...
  document.title =
    document.title + " " + window.initialTabInfo.type + " " + window.modelId;
  ...
  if (
    requestedWitness !== undefined &&
    requestedWitness !== null &&
    requestedWitness.length > 0
  ) {
    UI.isLoading(true);
...
let Strings = {
  ...
  modelWillBeErased:
    "This operation will overwrite your current model. Do you want to continue?",
