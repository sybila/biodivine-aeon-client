  ...
  Connection: {
    ...
    _updateComputationStatus(status, response) {
      ...
      if (ComputeEngine.Computation._computationType == "control") {
        ComputeEngine.Computation.Control.getControlComputationStatus(
          (e, r) => {
            if (e !== undefined) {
              console.log(e);
            ...
            if (r != undefined) {
              UI.UpdateStatus.updateComputeEngineStatus(status, {
                type: "control",
                timestamp:
                  r.isRunning == true
                    ? r.elapsed
                    : r.computationStarted + r.elapsed,
    ...
    Control: {
      ...
      _startControlComputation(aeonString, callback = undefined) {
        ...
        ComputeEngine._backendRequest(
          `/start_control_computation/${oscillation}/${this._minRobustness}/${this._maxSize}/${this._numberResults}`,
          (error, response) => {
            if (callback !== undefined) {
              callback(error, response);
            }
      ...
      _cancelControlComputation(callback = undefined) {
        return ComputeEngine._backendRequest(
    ...
    _startComputationErrors(aeonString) {
      ...
      if (this._computeControl && PhenotypeEditor.isPhenotypeEmpty()) {
        Warning.displayWarning(
          "Phenotype is empty. Add variables into the phenotype before performing control computation.",
        );
        return false;
      }
