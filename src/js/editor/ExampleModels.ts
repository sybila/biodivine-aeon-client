...
/*
    Just a bunch of example models that we can load into the editor for testing/demos.

    TODO: Replace this with models dynamically fetched from GitHub.
    TODO: Add recently opened feature.
*/

import Import from "./Import";

export let Examples: {
    init: (buttons: HTMLElement) => void,
    g2a: string,
    g2b: string,
    orlando: string,
    irons: string,
    ...
    init: function(buttons: HTMLElement): void {        
        let button_elements = buttons.getElementsByTagName("button");
        for (let i = 0; i < button_elements.length; i++) {
            const button = button_elements[i] as HTMLElement;
            if (typeof button.dataset["example"] === "string") {
                button.onclick = () => {
                    button.blur();  // Remove focus.
                    Import.importAeonModel(Examples[button.dataset["example"]]);
                };
            }
        }
    },
